...
Entity::HitTest(Rect _ent_rect, Rect _rect, char _tile){
    ...
    if (
        (_ent_rect.pos.x > (_rect.pos.x + _rect.size.x)) || // is the left side greater than the other rect's right side?
        ((_ent_rect.pos.x + _ent_rect.size.x) < _rect.pos.x) ||       // is the right side less than the other rect's left side?
        (_ent_rect.pos.y > (_rect.pos.y + _rect.size.y)) || // is the lower side greater than the other rect's upper side?
        ((_ent_rect.pos.y + _ent_rect.size.y) < _rect.pos.y))         // is the upper side less than the other rect's lower side?
    {
        ...
        if((_tile == '2' || _tile == '3')){
            ...
            pos.y = _rect.pos.y-_rect.size.y;
            vel.y = 0.0f;
            m_is_grounded = true;
        }
    }
...
void
Entity::FindSlopeTop(Rect _ent_rect, Rect _rect, char _tile){
    ...
    {
        if((_tile == '2' || _tile == '3')){
            if(floor((_rect.pos.y)/32) == floor((_ent_rect.pos.y)/32)){
                std::vector<int> valid_heights;
